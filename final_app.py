...
uploaded_files = st.file_uploader("Upload your PDF file/files", type=['pdf'], accept_multiple_files=True)

if api_key:
    if st.button("Document Embedding") and uploaded_files:
        if not os.path.exists("temp"):
            os.makedirs("temp")
        vector_embeddings(uploaded_files)
        st.write("FAISS Vector Store DB is ready using NVIDIA Embeddings")
else:
    st.warning("[-] Please enter the NVIDIA API key")
